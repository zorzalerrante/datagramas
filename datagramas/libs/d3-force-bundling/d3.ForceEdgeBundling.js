...
This version has been modified by Eduardo Graells-Garrido (@carnby) to adapt it for usage with matta:
* The library uses the graph format from matta's prepare_graph function.
...
define(["d3"], function(d3) {
	var ForceEdgeBundling = function(){
		...
		function edge_midpoint(e){
			var middle_x = (e.source.x + e.target.x) / 2.0;
			var middle_y = (e.source.y + e.target.y) / 2.0;
		...
		function filter_self_loops(edgelist){
			...
			for(var e=0; e < edgelist.length; e++){
				if(edgelist[e].source.x != edgelist[e].target.x  ||
				   edgelist[e].source.y != edgelist[e].target.y ){ //or smaller than eps
		...
		function update_edge_divisions(P){
			for(var e_idx=0; e_idx < data_edges.length; e_idx++){
				...
				if( P == 1 ){
					subdivision_points_for_edge[e_idx].push(data_edges[e_idx].source); // source
					subdivision_points_for_edge[e_idx].push(edge_midpoint(data_edges[e_idx])); // mid point
					subdivision_points_for_edge[e_idx].push(data_edges[e_idx].target); // target
				}else{
					...
					new_subdivision_points.push(data_edges[e_idx].source); //source
